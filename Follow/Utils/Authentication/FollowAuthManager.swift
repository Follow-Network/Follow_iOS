...
protocol FollowSessionListener {
...
enum FollowAuthorization: ResourceType {
    ...
    var baseURL: URL {
        guard let url = URL(string: "https://thefollow.org") else {
            fatalError("FAILED: https://thefollow.org")
        }
        return url
    }
...
class FollowAuthManager {
    ...
    var delegate: FollowSessionListener!
    ...
    private let follow: TinyNetworking<FollowAuthorization>
    ...
    init(clientID: String,
         ...
         follow:  TinyNetworking<FollowAuthorization> = TinyNetworking<FollowAuthorization>()) {
    ...
    public func accessToken(with code: String, completion: @escaping (String?, Error?) -> Void) {
        follow.request(resource: .accessToken(withCode: code)) { [unowned self] response in
            switch response {
